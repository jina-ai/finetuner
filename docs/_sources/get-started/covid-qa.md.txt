...
```python
from tests.data_generator import qa_blob_doc_generator
```

`qa_blob_doc_generator` is a generator that yields every question as a `Document` object. 
...
finetuner.fit(
   embed_model,
   train_data=qa_blob_doc_generator,
   interactive=True)
```
