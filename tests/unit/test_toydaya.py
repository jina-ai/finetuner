...
from finetuner.toydata import generate_qa_match, generate_fashion_match


def test_qa_data_generator():
    qa_docs = generate_qa_match(num_total=10)
    for d in qa_docs:
...
def test_train_test_generator():
    fmdg_train = generate_fashion_match(is_testset=True, num_total=10)
    fmdg_test = generate_fashion_match(is_testset=False, num_total=10)
...
def test_doc_generator():
    for d in generate_fashion_match(num_total=1000):
...
def test_fashion_documentarray():
    da = generate_fashion_match(num_total=10)
    assert len(da) == 10


def test_qa_documentarray():
    da = generate_qa_match(num_total=10, num_neg=3)
    assert len(da) == 10
